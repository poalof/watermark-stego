...
function msgLen=F4_embed(srcPath, destPath, msgPath)
...
for row = 1:rows
    for col = 1:cols
        ...
        if encodedImage(row,col) == 0 || (encodedImage(row,col)==1 && bitList(bitIndex)==0 || encodedImage(row,col)==-1 && bitList(bitIndex)==1)
            encodedImage(row,col) = 0;
            continue; % coeff为零的跳过
        end
        % 将文本信息的一个二进制位嵌入到像素的最低有效位中
        if bitList(bitIndex) == 0
            if encodedImage(row,col) < 0 && mod(encodedImage(row,col),2)==0 % 负数嵌入0 改为奇数
                encodedImage(row,col) = encodedImage(row,col)+1;
            elseif encodedImage(row,col) > 0 && mod(encodedImage(row,col),2)==1
                encodedImage(row,col) = encodedImage(row,col)-1;
            end
        else
            if encodedImage(row,col) < 0 && mod(encodedImage(row,col),2)==1 % 负数嵌入1 改为偶数
                encodedImage(row,col) = encodedImage(row,col)+1;
